...
CONFIG = {
    ...
    "model_path": "models/urgency_classifier.pkl",
    "vectorizer_path": "models/urgency_vectorizer.pkl",
    "max_features": 10000, # Increased for larger data
    "test_size": 0.2,
    "random_state": 42,
    "max_iter": 2000,
}

class UrgencyModelTrainer:
    ...
    def load_and_preprocess(self):
        ...
        for ds in self.config["datasets"]:
            ...
            # Map labels
            df["urgency"] = df[ds["label_col"]].map(ds["map"])
            
            # Keep only needed columns
            df = df[["full_text", "urgency"]].dropna()
            all_dfs.append(df)
    ...
    def train(self):
        ...
        print(f"Training urgency model (Samples: {X_train.shape[0]})...")
...
if __name__ == "__main__":
    trainer = UrgencyModelTrainer(CONFIG)
    try:
        trainer.train()
